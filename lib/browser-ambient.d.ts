...
// declared (via `websql-spec-ambient.d.ts`, alongside this file) on
// `Window`/`WorkerUtils`, and this project doesn't pull in DOM lib types.
//
...
// require `websql-spec-ambient.d.ts` (for `WindowDatabase`) to also be
// reachable there, which it isn't (see that file's own comment). This
