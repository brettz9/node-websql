...
function runBatch (self, batch) {
  ...
  const {readOnly} = self._task;

  self._websqlDatabase._db.exec(batch, readOnly, function (err, results) {
    /* c8 ignore next -- Should not err or fail to have results? */
    if (err || !results) {
