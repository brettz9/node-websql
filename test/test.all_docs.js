        ...
        function () {
          db.allDocs(function (err, result) {
            ...
            for (const row of rows) {
              row.id.should.be.at.least('0');
              row.id.should.be.at.most('4');
            }
            db.allDocs({
              startkey: '2',
              include_docs: true
            ...
            db.get('3', function (err, doc) {
              ...
              db.put(doc, function () {
                db.changes({
                  ...
                  const ids = changes.results.map(function (c) {
                    return c.id;
                  });
            ...
            db.put(conflictDoc2, {new_edits: false}, function () {
              db.get('3', function (err, winRev) {
                ...
                db.changes({
                  ...
                  changes.results.map(function (x) {
                    return x.id;
                  }).sort().
                    should.deep.equal(['0', '1', '2', '3'],
                      'all ids are in _changes');

                  const result = changes.results.find(function (row) {
                    return row.id === '3';
                  });
