  ...
  describe('suite2 test.attachments.js-' + adapter, function () {
    ...
    it('#2858 {binary: true} in allDocs() 2', function () {
      ...
      return db.bulkDocs(docs).then(function () {
        return db.allDocs({
          ...
          const savedDocs = res.rows.map(function (x) {
            return x.doc;
          });
          return PouchDB.utils.Promise.all(docs.map(function (doc) {
            const atts = doc._attachments;
            const attName = atts && Object.keys(atts)[0];
            const expected = atts && atts[attName];
            const savedDoc = savedDocs.find(function (x) {
    ...
    it('#2858 {binary: true} in live changes', function () {
      ...
      return db.bulkDocs(docs).then(function () {
        return new PouchDB.utils.Promise(function (resolve, reject) {
          ...
          let promise = PouchDB.utils.Promise.resolve();
          let done = 0;
    ...
    it('Measures length correctly after put()', function () {
      ...
      return db.put(binAttDoc).then(function () {
        ...
        const {digest} = doc._attachments['foo.txt'];
        const validDigests = [
    ...
    it.skip('#3074 non-live changes()', function () {
      ...
      return db.bulkDocs(docs).then(function () {
        ...
        const attachments = res.results.sort(function (left, right) {
    ...
    it('#3881 filter extraneous keys from _attachments', function () {
      ...
      return db.put({
        ...
        const keys = Object.keys(doc._attachments['foo.txt']).filter(function (x) {
    ...
    it.skip('#2771 allDocs() 4, mix of atts and no atts', function () {
      ...
      return db.bulkDocs(docs).then(function () {
        ...
        const attachments = res.rows.map(function (row, i) {
          const {doc} = row;
          if (i % 2 === 1) {
            delete doc._attachments['foo.png'].revpos;
            return doc._attachments;
      ...
      return db.put({
        ...
        _attachments: {
          1: {
            ...
            data: testUtils.btoa('1\u{0}2\u{1}3\u{2}')
          }
        }
      }).then(function () {
        return db.get('doc');
        ...
        doc._attachments['2'] = {
          content_type: 'application/octet-stream',
          data: testUtils.btoa('3\u{0}2\u{1}1\u{2}')
        };
        return db.put(doc);
      }).then(function () {
        ...
        bin.should.equal('1\u{0}2\u{1}3\u{2}');
        return db.getAttachment('doc', '2');
      }).then(function (att) {
        att.type.should.equal('application/octet-stream');
        return testUtils.readBlobPromise(att);
      }).then(function (bin) {
        bin.should.equal('3\u{0}2\u{1}1\u{2}');
      });
    });
      ...
      db.bulkDocs({docs}, function () {
        db.allDocs({include_docs: true}, function (err, res) {
          for (const [i, doc] of docs.entries()) {
            const attachmentsNb = typeof doc._attachments !== 'undefined'
          ...
          db.changes({
            ...
            const i = Number(change.id.slice(3));
            if (i === 0) {
              should.not.exist(res.rows[0].doc._attachments,
              ...
              const attachmentsNb =
                typeof docs[i]._attachments !== 'undefined'
                  ? Object.keys(docs[i]._attachments).length
                  : 0;
              for (let j = 0; j < attachmentsNb; j++) {
                res.rows[i].doc._attachments['att' + j].stub.should.
    ...
    it('Test putAttachment with invalid base64', function () {
      const db = new PouchDB(dbs.name);
      return db.putAttachment('doc', 'att', null, '\u{65E5}\u{672C}\u{8A9E}', 'text/plain').
    ...
    it('Test postAttachment with PNG then bulkDocs', function (done) {
      ...
      db.put({_id: 'foo'}, function () {
        db.get('foo', function (err, doc) {
          const {data} = pngAttDoc._attachments['foo.png'];
    ...
    it('Test synchronous putAttachment with no text data', function (done) {
      ...
      db.putAttachment('a', 'foo2.txt', '', '', 'text/plain', function (err) {
        ...
        db.get('a', {attachments: true}, function (err, doc) {
          ...
          // see https://forums.mozillazine.org/viewtopic.php?p=6318215#p6318215
    ...
    it('putAttachment and getAttachment with plaintext', function (done) {
      ...
      db.put({_id: 'foo'}, function () {
        db.get('foo', function (err, doc) {
          const {data} = binAttDoc._attachments['foo.txt'];
          const blob = testUtils.binaryStringToBlob(testUtils.atob(data),
            'text/plain');
    ...
    it('putAttachment in new doc with base64', function () {
      ...
      return db.putAttachment('foo', 'att', 'Zm9v', 'text/plain').then(function () {
        ...
        doc._attachments.att.content_type.should.match(/^text\/plain/v);
    ...
    it(String.raw`#3008 test correct encoding/decoding of \u0000 etc.`, function () {
      const base64 =
    ...
    function () {
      ...
      it('Multiple attachments replicate, different docs (#2698)', function () {
        ...
        return remote.bulkDocs(docs).then(function () {
          ...
          const attachments = docs.map(function (doc) {
            delete doc._attachments['foo.txt'].revpos;
            delete doc._attachments['foo.txt'].digest;
            return doc._attachments;
          });
          ...
          return PouchDB.utils.Promise.all([db, remote].map(function (pouch) {
            return pouch.get('test1', {attachments: true}).then(function (doc) {
              ...
              filenames.forEach(function (filename) {
                const {data} = doc._attachments[filename];
      ...
      it('3955 race condition in put', function (done) {
        ...
        db.put(doc).then(function () {
          ...
          const digests = Object.values(doc._attachments).map(function (value) {
            return value.digest;
          });
          if (isUnique(digests)) {
            done();
