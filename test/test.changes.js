  ...
  describe('test.changes.js-' + adapter, function () {
    ...
    it('#3136 style=all_docs', function () {
      ...
      return chain.then(function () {
        ...
        const ids = res.results.map(function (x) {
          return x.id;
        });
    ...
    it('#4191 revs_diff causes endless loop', function () {
      ...
      return db.bulkDocs({
        new_edits: false,
        docs: [{_id: '799', _rev: '1-d22'}]}).then(function () {
        return db.bulkDocs({
          new_edits: false,
    ...
    it('#3136 winningRev has a lower seq, style=all_docs 2', function () {
      ...
      return chain.then(function () {
        ...
        let chain2 = PouchDB.utils.Promise.resolve();
    ...
    it('#2569 Non-live doc_ids filter', function () {
      ...
      return db.bulkDocs(docs).then(function () {
        ...
        const ids = changes.results.map(function (x) {
          return x.id;
        });
    ...
    it('#2569 Live doc_ids filter', function () {
      ...
      return db.bulkDocs(docs).then(function () {
        ...
        const ids = changes.map(function (x) {
          return x.id;
        });
...
describe('changes-standalone', function () {
  it('Changes reports errors', function (done) {
    const db = new PouchDB('https://infiniterequest.com', {skipSetup: true});
    db.changes({
      timeout: 1000
    }).on('error', function (err) {
      should.exist(err);
