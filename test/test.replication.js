  ...
  describe('suite2 test.replication.js-' + adapters[0] + '-' + adapters[1],
    function () {
      ...
      function simplifyChanges (res) {
        let changes = res.results.map(function (change) {
      ...
      it.skip('Test pull replication with many conflicts', function (done) {
        ...
        for (let i = 0; i < numRevs; i++) {
          const rev = '1-' + PouchDB.utils.uuid(32, 16).toLowerCase();
          ...
          PouchDB.replicate(
            ...
            db.info(function (err, info) {
              ...
              db.get('doc', {open_revs: 'all'}, function (err, docs) {
                should.not.exist(err);
                const okDocs = docs.filter(function (doc) {
                  return doc.ok;
                });
      ...
      it('#3136 open revs returned correctly 1', function () {
        ...
        const doc = {_id: 'foo'};
        let chain = PouchDB.utils.Promise.resolve().then(function () {
          return db.put(doc);
        });
      ...
      it('#3136 winningRev has a lower seq', function () {
        ...
        const db2 = new PouchDB(dbs.remote);
        const tree = [
          [
            {_id: 'foo', _rev: '1-a',
        ...
        let chain = PouchDB.utils.Promise.resolve();
      ...
      it('Testing allDocs with some conflicts (issue #468)', function (done) {
        ...
        const doc = {_id: 'foo', _rev: '1-a', value: 'generic'};
      ...
      it('Test basic live pull replication', function (done) {
        ...
        remote.bulkDocs({docs}, {}, function () {
          ...
          const rep = db.replicate.from(dbs.remote, {
            live: true
          }).on('complete', isFinished);
          const changes = db.changes({
            live: true
        ...
        db.bulkDocs({docs}).then(function () {
          db.replicate.to(dbs.remote).
            on('complete', function (res) {
              should.exist(res);
              // eslint-disable-next-line sonarjs/no-clear-text-protocols -- deliberately unreachable test target
      ...
      it('live replication, starting offline', function () {
        const db = new PouchDB(dbs.name);
        const remote = new PouchDB(dbs.remote);
        const {Promise: LocalPromise} = PouchDB.utils;
      ...
      it('Replicates deleted docs w/ delay (issue #2636)', function () {
        ...
        return db.post({}).then(function (res) {
          ...
          return waitForChange(remote, function () {
            return true;
          });
        }).then(function () {
          return db.remove(doc);
        ...
        db1.post(adoc, function () {
          PouchDB.replicate(db1, db2).on('complete', function () {
            db1.destroy(function () {
              const fresh = new PouchDB(db1name);
      ...
      it('issue #1001 cb as 3rd argument', function (done) {
        PouchDB.replicate('https://example.com', dbs.name, function (err) {
          should.exist(err);
          done();
        });
      });
      ...
      it('#3999-2 should start over if no common session is found', function () {
        const source = new PouchDB(dbs.remote);
        let mismatch = false;
        ...
        source.put = function (doc, ...rest) {
        // We need the checkpoint id so we can inspect it later
          if ((/local/v).test(doc._id)) {
            checkpointCount++;
            checkpoint = doc._id;
          }

          if (!writeStrange || checkpointCount < 1) {
            return Reflect.apply(putte, this, [doc, ...rest]);
          }
        ...
        source.changes = function (opts, ...rest) {
          if (mismatch) {
          ...
          return changes.apply(source, [opts, ...rest]);
        };
      ...
      it('#3999-4 should "upgrade" an old checkpoint', function () {
        ...
        // 1. This is where we fake the mismatch:
        const putter = function (doc, ...rest) {
        // We need the checkpoint id so we can inspect it later
          if ((/local/v).test(doc._id)) {
            checkpointCount++;
            checkpoint = doc._id;
        ...
        function (done) {
          ...
          db.bulkDocs({docs}, {new_edits: false}, function () {
            ...
            remote.bulkDocs = function (docs, opts, callback) {
              if (typeof callback !== 'function') {
                return PouchDB.utils.Promise.reject(new Error('simulated bulkDocs failure'));
              }
              callback(new Error('simulated bulkDocs failure'));
              return undefined;
            };
      ...
      it.skip('doc count after multiple replications', function (done) {
        ...
        function rebuildDocuments (db, docs, callback) {
          db.allDocs({include_docs: true}, function (err, response) {
            let count = 0;
            const limit = response.rows.length;
            if (limit === 0) {
        ...
        const workflow = function (name, remote, x) {
      ...
      it.skip('should work with a read only source', function (done) {
        const src = new PouchDB(dbs.name);
        const target = new PouchDB(dbs.remote);
        const err = {
          message: '_writer access is required for this request',
          name: 'unauthorized',
          status: 401
      ...
      it('#2970 should replicate remote database w/ deleted conflicted revs',
        function (done) {
          ...
          const a_doc = {
            ...
            _revisions: {
              start: a_rev_num,
              ids: [a_latest, a_burner, a_conflict, ...uuids]
            }
          };

          // conflicted deleted branch
          const b_conflict = uuid();
          const b_deleted = uuid();
          ...
          const b_doc = {
            ...
            _revisions: {
              start: b_rev_num,
              ids: [b_deleted, b_conflict, ...uuids]
            }
          };
      ...
      it.skip('#3171 Forbidden validate_doc_update error message',
        function (done) {
          testUtils.isCouchDB(function (isCouchDB) {
            ...
            return remote.put(ddoc).then(function () {
              const docs = [{foo: 'string'}, {}, {foo: 'object'}];
              return db.bulkDocs({docs});
            }).then(function () {
              return db.replicate.to(dbs.remote);
            }).then(function (res) {
      ...
      it('#3270 triggers "denied" events', function (done) {
        testUtils.isCouchDB(function (isCouchDB) {
          ...
          return remote.put(ddoc).then(function () {
            ...
            const replication = db.replicate.to(dbs.remote);
      ...
      it('#3606 - live replication with filtered ddoc', function () {
        ...
        {_id: 'a', name: 'anna'},
        {_id: 'b', name: 'barbara'},
        {_id: 'c', name: 'charlie'}]).then(function () {
          return new LocalPromise(function (resolve, reject) {
            const replicate = remote.replicate.to(db, {
      ...
      it('#3270 triggers "change" events with .docs property', function (done) {
        ...
        db.bulkDocs({docs}, {}).then(function () {
          ...
          replication.on('change', function (change) {
            replicatedDocs = [...replicatedDocs, ...change.docs];
          });
          return replication;
        }).
          then(function () {
      ...
      it('4094 cant fetch server uuid', function (done) {
        const {ajax} = PouchDB.utils;

        PouchDB.utils.ajax = function (opts, cb, ...rest) {
          const uri = PouchDB.utils.parseUri(opts.url);
          if (uri.path === '/') {
            cb(new Error('flunking you'));
          } else {
            Reflect.apply(ajax, this, [opts, cb, ...rest]);
          }
        };
        ...
        const changes = db.changes({live: true}).on('change', function () {
          rep.cancel();
          changes.cancel();
        }).on('complete', complete);
      ...
      it('#4293 Triggers extra replication events', function (done) {
        ...
        const push = remote.replicate.from(db, {live: true}).
          on('paused', paused).
          on('complete', complete);

        const pull = db.replicate.from(remote, {live: true}).
          on('change', change).
          on('complete', complete);
      ...
      it('#4276 Triggers paused error', function (done) {
        if (!(/http/v).test(dbs.remote) || (/http/v).test(dbs.name)) {
          done();
          return;
        }
        ...
        db.bulkDocs([{foo: 'bar'}]).then(function () {
          const repl = db.replicate.to(remote, {live: true, retry: true});
      ...
      it('Heartbeat gets passed', function () {
        ...
        const {ajax} = PouchDB.utils;
        PouchDB.utils.ajax = function (opts, ...rest) {
          if ((/heartbeat/v).test(opts.url)) {
            seenHeartBeat = true;
          }
          Reflect.apply(ajax, this, [opts, ...rest]);
        };
      ...
      it('Timeout gets passed', function () {
        ...
        // the http adapter takes 5s off the provided timeout
          if ((/timeout=15000/v).test(opts.url)) {
            seenTimeout = true;
          }
          Reflect.apply(ajax, this, [opts, ...rest]);
        };
